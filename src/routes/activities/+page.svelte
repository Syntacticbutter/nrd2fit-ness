<script lang="ts">
	...
	const clubAct = async () => {
		...
		try {
			let api = `https://www.strava.com/api/v3/clubs/${club_id}/activities?page=${page}&per_page=${pageSize}`;
			...
			<div class="lg:flex lg:justify-center flex-wrap">
				{#each items as i}
					<section class="w-96 p-4 rounded-lg m-4 border border-solid dark:border-neutral-800">
						<h1 class="capitalize">{i.athlete.firstname} {i.athlete.lastname}</h1>
						<h2 class="font font-semibold text text-lg">{i.name}</h2>
						<h3 class="font font-light text text-xs">{i.sport_type}</h3>
